...
module.exports = RFDevice => class Dimmer extends Kaku(RFDevice) {
	...
	static payloadToData(payload) {
		...
		if (payload && payload.indexOf(2) === -1) {
			...
			const data = {
				...
				state: 1, // Since there is a dim level, the light should be turned on.
				onoff: true,
	...
	// Seems to be not needed anymore. Without dim property the signal is handled as a onoff signal.
